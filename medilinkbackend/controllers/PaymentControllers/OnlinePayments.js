...
const onlinePaymentSchema = require("../../models/PaymentModels/OnlinePaymentSchema");
const BillingSchema = require("../../models/PaymentModels/BillingSchema");
...
exports.handlePaymentNotification = (req, res) => {
  const {
    merchant_id,
    order_id, // Assume this holds the billNo
    payment_id,
    payhere_amount,
    payhere_currency,
    status_code,
  ...
  const onlinePayment = new onlinePaymentSchema({
    ...
    card_no,
    card_expiry,
    sv: local_md5sig === md5sig, // sv will be true if local_md5sig matches the provided md5sig
  });
  ...
  onlinePayment
    .save()
    .then((savedPayment) => {
      // Check if status_code is 2 (indicating payment success) and sv is true
      if (status_code === "2" && savedPayment.sv === true) {
        // Find the bill from the BillingSchema using the order_id (as billNo)
        BillingSchema.findOneAndUpdate(
          { billNo: order_id }, // Assuming order_id holds the billNo
          { paymentStatus: "Paid" }, // Set the paymentStatus to true
          { new: true } // Return the updated document
        )
          .then((updatedBill) => {
            if (updatedBill) {
              console.log("Billing payment status updated:", updatedBill);
            } else {
              console.error("No bill found with the given order_id.");
            }
          })
          .catch((error) => {
