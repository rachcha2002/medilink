...
function MedicalRecords() {
    ...
    const fetchRecords = async () => {
      try {
        const response = await fetch(
          "http://localhost:5000/api/medicalinfo/medical-records"
        );
        if (!response.ok) {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(
        `http://localhost:5000/api/medicalinfo/medical-records/${id}`,
        {
          method: "DELETE",
        }
      );
  ...
  return (
    <main id="main" className="main">
      <PageTitle title="Medical Records" url="/mltstaff/medical-records" />
      <Container
        className="p-4"
        style={{ background: "rgba(255, 255, 255, 0.4)", borderRadius: "10px" }}
      >
        <h3 className="mt-4">Medical Records Table</h3>

        {error && (
          <Alert variant="danger">Error fetching records: {error}</Alert>
        )}
